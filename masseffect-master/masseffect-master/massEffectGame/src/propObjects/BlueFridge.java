...
/**
 * @author joshua hamilton-brown the BlueFridge object is a prop object that is used in gameplay for hiders and seekers to interact with.
 * The blue fridge prop object contains final variables WIDTH and HEIGHT to set the dimensions in pixels of the prop when drawn on screen.
 * The final String blueFridgeFilePath designates the file path where the .png file is stored for drawing the prop on screen.
 ...
 */
public class BlueFridge extends Prop {
	
	public static final int WIDTH = 47, HEIGHT = 82;//the BlueFridge is the biggest prop by area so it has the maximum health of 100, other props have health as a fraction of the fridge depending on its area.
	public static final String blueFridgeFilePath = "/assets/BlueFridge.png";
	public static int HEALTH = 100  * (WIDTH*HEIGHT) / (WIDTH*HEIGHT);
	 ...
	 */
	public BlueFridge(int x, int y, PropObjectID id, Pane pane, ObjectController objControl) {
		super(x, y, HEALTH, "/assets/BlueFridge.png", WIDTH, HEIGHT, id, pane, objControl);
		this.filePath = getClass().getResource(blueFridgeFilePath).toString();
		this.height = HEIGHT;
		this.width = WIDTH;
		this.health = HEALTH;
